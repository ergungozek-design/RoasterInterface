...
class FolderTile(ButtonBehavior, BoxLayout):
    ...
    def on_touch_up(self, touch):
        ...
        # short press
        if self.collide_point(*touch.pos) and self.on_short_press is not None:
...
class ProfileScreen(Screen):
    ...
    # ---------------------------------------------------------
    # ✅ TextKeypad open (your custom keypad)
    # ---------------------------------------------------------
    def _open_text_keypad(self, title: str, initial_text: str, on_ok, max_len: int = 24):
        # TextKeypadPopup default fullscreen=True already :contentReference[oaicite:1]{index=1}
        TextKeypadPopup(
            title=title,
            initial_text=initial_text or "",
            max_len=max_len,
            fullscreen=True,
            on_ok=lambda txt: on_ok(self._sanitize_name(txt)),
    ...
    def _confirm_delete_folder(self, folder_name: str):
        ...
        # ---- Title + folder name (büyük, okunaklı) ----
        msg = Label(
            text="Delete folder?",
            font_size="20sp",
            bold=True,
        ...
        pop = self._dark_popup(card, w=520, h=300)  # ✅ daha iyi boyut
    ...
    def show_files(self, folder_name: str):
        ...
        for i in range(self.MAX_SLOTS):
            ...
            if tile.is_empty:
                tile.on_short_press = self._empty_file_clicked  # ✅ boş profile tıklayınca isim iste
                tile.on_long_press = None
            else:
                tile.on_short_press = self._file_clicked
                tile.on_long_press = self._file_long_pressed  # ✅ 1sn basınca menü
    ...
    def _file_clicked(self, tile: FolderTile):
        ...
        if hasattr(detail, "load_profile"):
            ...
            #detail.load_profile(f"{folder_name}/{profile_name}", data)
        else:
    ...
    def on_back(self):
        ...
        if self.manager:
            if self.manager.has_screen("live_roast"):
